'use client'

import Image from 'next/image'

interface LoginScreenProps {
  sessionExpired: boolean
  onDismissSessionExpired: () => void
  isReady: boolean
  isLoading: boolean
  onAuthClick: () => void
}

export function LoginScreen({
  sessionExpired,
  onDismissSessionExpired,
  isReady,
  isLoading,
  onAuthClick
}: LoginScreenProps) {
  return (
    <div className="login-container">
      {sessionExpired && (
        <div className="session-expired-banner" role="alert">
          <span className="session-expired-icon">⚠️</span>
          <span>Sesión expirada. Por favor, vuelva a ingresar.</span>
          <button type="button" className="session-expired-dismiss" onClick={onDismissSessionExpired} aria-label="Cerrar aviso">×</button>
        </div>
      )}
      <div className="login-left">
        <div className="welcome-content">
          <div className="logo-wrapper">
            <Image 
              src="/Clarín_logo.svg.png" 
              alt="Clarín Logo" 
              width={180} 
              height={50}
              priority
            />
          </div>
          <h1>Bienvenido a</h1>
          <h2>Relevamiento de PDV</h2>
          <p>
            Accede con tu cuenta de Google para consultar y gestionar los datos del relevamiento de manera segura.
          </p>
        </div>
        
        <div className="clouds-decoration">
          <div className="cloud cloud-1"></div>
          <div className="cloud cloud-2"></div>
          <div className="cloud cloud-3"></div>
        </div>
      </div>

      <div className="login-right">
        <div className="login-card">
          <h3>Iniciar Sesión</h3>
          <p>Usa tu cuenta de Google para acceder al sistema</p>
          
          <button 
            className={`google-btn ${isLoading ? 'loading' : ''}`}
            onClick={onAuthClick}
            disabled={!isReady || isLoading}
          >
            {isLoading ? (
              <div className="spinner"></div>
            ) : (
              <svg viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
              </svg>
            )}
            {isLoading ? 'Conectando...' : 'Continuar con Google'}
          </button>
          
          {!isReady && (
            <div style={{ textAlign: 'center', marginTop: '1rem', color: 'var(--gray-300)' }}>
              <small>Cargando...</small>
            </div>
          )}
        </div>
        
        <div className="footer-text">
          <p>© 2026 Clarín - Todos los derechos reservados</p>
        </div>
      </div>
    </div>
  )
}
